...
```{r problemPageDepends, eval=FALSE}
...
```{r biocDownloadStats}
stats <- try(biocDownloadStats(), silent=TRUE)
if (!inherits(stats, "try-error")) {
    head(stats)
} else {
    message("Download stats not available at this time.")
}
```
