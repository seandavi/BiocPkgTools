...
the build status of packages matching a given author *regex* supplied to the `authorPattern` argument. 
...
a large number of these.  You can use the `dependsOn` argument to produce the summary report
...
their repositories.  Access to these counts is provided by the `anacondaDownloadStats` function:

```{r}
head(anacondaDownloadStats())
```

Note that Anaconda do not provide counts for distinct IP addresses, but this column is included for
compatibility with the Bioconductor count tables.
...
details are available in bulk for all included packages. The `biocPkgList` returns
