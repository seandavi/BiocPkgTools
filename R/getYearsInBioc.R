utils::globalVariables(c(
    "Release", "approx_years_in", "category",
...
#'
#' @description This function determines the number of years a package has been
...
getPkgYearsInBioc <- function(pkglist = NULL) {
    ...
    rid <- BiocFileCache::bfcquery(
        bfc, "bioc.pkg.years.info", "rname", exact = TRUE
    )[["rid"]]
    if (!length(rid)) {
        pkginfo <-
            BiocFileCache::bfcnew(bfc, "bioc.pkg.years.info", ext = ".Rdata")
    } else {
...
## Internal: reformat information to be per package instead of per release.
## This will also calculate out when package first appears and when if
## applicable last appears (if it was removed).
.formatBiocYearsDF <- function(NeededYearsData) {
    config <- NeededYearsData[["config"]]
    manifestDF <- NeededYearsData[["manifestDF"]]

    manifestDFUnlist <- manifestDF |>
        unnest_longer(packages) |>
        mutate(
            release = Release, version = major * 100 + minor,
            category = packages$category, package = packages$package,
            .keep = "none"
        )
    ...
    manifestByPkg <- manifestDFUnlist |>
        group_by(package) |>
        summarize(
            category = category[which.max(version)],
    ...
    names(release_dates_date) <-
        c(names(release_dates), config[["devel_version"]])
    since <-
        as.numeric(round(((Sys.Date() - release_dates_date) / 365.25) * 2) / 2)
    names(since) <- c(names(release_dates), config[["devel_version"]])

    YearsInBiocDF <- manifestByPkg |>
        mutate(
            first_version_release_date =
                release_dates_date[first_version_available],
            last_version_release_date =
                release_dates_date[last_version_available],
            approx_years_in = ifelse(
                last_version_available == config[["devel_version"]],
                since[first_version_available],
                NA
            ),
            years_before_rm = ifelse(
                last_version_available == config[["devel_version"]],
                NA,
                as.numeric(
                    round(
        ...
        select(
            package, category, first_version_available,
            first_version_release_date, approx_years_in,
            last_version_available, last_version_release_date,
            years_before_rm
        )
    return(YearsInBiocDF)
}
...
.updateYearsInBioc <- function(NeededYearsData, cachepathinfo) {
    ...
    neededReleases <-
        setdiff(current_bioc_ver, c("1.6", "1.7", manifestDF$Release))
    if (length(neededReleases) > 0) {
        bioc_ver <- data.frame(Release = neededReleases)
        newEntries <- cbind(
            bioc_ver,
            list_rbind(lapply(
                strsplit(bioc_ver$Release, ".", fixed = TRUE),
                (\(u) data.frame(
                    major = as.integer(u[1]),
...
.generateBiocYearsInData <- function(cachefilepath) {
    message(
        "Internet Access Required For Data Generation \n",
...
.genBiocManifestDF <- function(major, minor) {
    manifest_template <- paste0(
        "https://www.bioconductor.org/packages/",
        "{version}/{category}/src/contrib/PACKAGES"
    )
